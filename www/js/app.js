...
.run(function($rootScope, $ionicPlatform, $ionicLoading, $ionicPopup, $cordovaSQLite) {
	...
	$ionicPlatform.ready(function() {
		...
		$cordovaSQLite.execute(db, "CREATE TABLE IF NOT EXISTS DetObservaciones(IdDetObservacion integer primary key,IdObservacion integer,IdEstandar integer,NumCompPositivos integer,NumCompObservados integer,Acciones text,IdEstadoDetObservacion integer,IdDetObservRemoto text,PrefijoRemoto text,FOREIGN KEY(IdEstandar) REFERENCES Estandares(IdEstandar),FOREIGN KEY(IdObservacion) REFERENCES Observaciones(IdObservacion))");
